$(document).ready(function () {
    var pageBuilder = {
        ...
        init: function () {
            ...
            var submit = document.querySelector("#updatePageForm_submit");
            var inputData = document.querySelector("#updatePageForm_data");
            pageBuilder.pageData = JSON.parse(inputData.value);
        ...
        constructPage: function () {
            ...
            newRowContainerTop = document.createElement("div");
            newRowContainerTop.className = "pageBuilder-container-hidden";
            newRowContainerTop.append(newRowBtnTop);
            pageBuilder.newTopRowBtn = newRowContainerTop;
            page.append(newRowContainerTop);
            pageBuilder.addEventToAddRowBtn(newRowBtnTop, newRowBtnTop.parentNode, "after");

            var first = pageData[pageData.page.first];
            if (null != first) pageBuilder.generatePage(first, pageData.page.first);

            newRowBtnBottom = document.createElement("div");
            newRowBtnBottom.className = "pageBuilder-btn-new-top";
            newRowContainerBottom = document.createElement("div");
            newRowContainerBottom.className = "pageBuilder-container-hidden";
            newRowContainerBottom.append(newRowBtnBottom);
        ...
        generatePage: function (element, id) {
            var type = element.type;
            var pageData = pageBuilder.pageData;

            if (type === "row") {
                var row = document.createElement("div");
                row.className = element.class;
                row.id = id;
                console.log(parseInt(id.split("-")[1]));
                if (pageBuilder.rowId <= id.split("-")[1]) pageBuilder.rowId = parseInt(id.split("-")[1]) + 1;

                element.node = row;
                pageBuilder.page.append(row);
                var first = pageData[element.first];
                if (null !== first) pageBuilder.generatePage(first, element.first);
            }

            if (type === "childRow") {
                console.log(pageData[element.parent]);
                var parent = pageData[element.parent].node;

                var col = document.createElement("div");
                var colInner = document.createElement("div");
                var row = document.createElement("div");

                col.className = element.class;
                colInner.className = "col-inner padding-0";
            ...
            if (type === "container") {
                ...
                container.id = id;
                console.log(parseInt(id.split("-")[1]));
                // console.log(pageBuilder.containerId < parseInt(id.split("-")[1]))
                if (pageBuilder.containerId <= id.split("-")[1])
                    pageBuilder.containerId = parseInt(id.split("-")[1]) + 1;
                ...
                edit.addEventListener("click", function () {
                    ...
                    var editor = pageBuilder.initEditor(container, element.content);
                    editor.setContent(element.content);
            ...
            pageBuilder.generatePage(pageData[next], next);
        },
        ...
        deleteContainer: function (container) {
            ...
            containerRow.childNodes.forEach((col) => {
                ...
                var container = col.firstChild.firstChild;
                pageData[container.id].class = col.className;
            });
            console.log(pageData);
        },
